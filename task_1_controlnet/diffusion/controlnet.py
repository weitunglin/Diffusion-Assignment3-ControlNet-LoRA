...
class ControlNetModel(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def from_unet(
        ...
        # this function is generated by claude.ai
        def copy_weights(source_model, target_model, module_names=None):
            source_dict = dict(source_model.named_modules())
            target_dict = dict(target_model.named_modules())

            if module_names is None:
                module_names = source_dict.keys()

            for name in module_names:
                if name in source_dict and name in target_dict:
                    target_dict[name].load_state_dict(source_dict[name].state_dict())

        copy_weights(unet, controlnet, ['conv_in', 'time_proj', 'time_embedding', 'down_blocks', 'mid_block'])
        ######## TODO (2) ########
    ...
    def forward(
        ...
        for i, b in enumerate(down_block_res_samples):
            b = self.controlnet_down_blocks[i](b)
        mid_block_res_sample = self.controlnet_mid_block(down_block_res_samples[-1])
        ######## TODO (3) ########
